...
for sheet in config:
  if sheet['name'] != 'all':
    ...
    for version in sheet.get('version', []):
      if HRA_VERSION in version.get(VERSION_FIELD, '') and 'sheetId' in version:
...
open('dist/ccf-asctb-drafts.json', 'w').write(requests.get(JSON_URL).text)
json.dump(bysheet, open('dist/ccf-asctb-drafts-all.json', 'w'), indent=2)

graphData = requests.get(GRAPH_URL).text
g = Graph()
g.parse(data=graphData, format='json-ld')
g.serialize('dist/ccf-asctb-drafts.owl', format='xml')
